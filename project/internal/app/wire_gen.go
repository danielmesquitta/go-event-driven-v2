...
func New() Service {
	...
	opsbookingList := opsbooking.NewList(opsBooking)
	get := opsbooking.NewGet(opsBooking)
	opsBookingHandler := handler.NewOpsBookingHandler(opsbookingList, get)
	echo := router.New(showHandler, ticketHandler, healthHandler, bookingHandler, opsBookingHandler)
	...
	onTicketBookingConfirmed := opsbooking.NewOnTicketBookingConfirmed(opsBooking)
	updateOpsBooking := bookingconfirmed.NewUpdateOpsBooking(onTicketBookingConfirmed)
	...
	onTicketRefunded := opsbooking.NewOnTicketRefunded(opsBooking)
	ticketrefundedUpdateOpsBooking := ticketrefunded.NewUpdateOpsBooking(onTicketRefunded)
	onTicketPrinted := opsbooking.NewOnTicketPrinted(opsBooking)
	ticketprintedUpdateOpsBooking := ticketprinted.NewUpdateOpsBooking(onTicketPrinted)
	onTicketReceiptIssued := opsbooking.NewOnTicketReceiptIssued(opsBooking)
	ticketreceiptissuedUpdateOpsBooking := ticketreceiptissued.NewUpdateOpsBooking(onTicketReceiptIssued)
